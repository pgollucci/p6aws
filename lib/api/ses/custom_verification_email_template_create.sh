...
p6_aws_ses_custom_verification_email_template_create() {
    ...
    p6_run_write_cmd aws ses create-custom-verification-email-template --template-name $template_name --from-email-address $from_email_address --template-subject $template_subject --template-content $template_content --success-redirection-url $success_redirection_url --failure-redirection-url $failure_redirection_url "$@"
}